...
input = '''light salmon bags contain 5 dotted olive bags, 4 wavy lavender bags.
...
for l in input.split('\n'):
    ...
    bagname, contents = l
    bagname = bagname.strip()
    contents = [re.sub(r'bag(s|)([\W]|)', '', b).strip().split(' ', 1) for b in contents.split(',')]
    contents = [(int(c[0]), c[1]) for c in contents if c[0].isdigit()]
    rulebook[bagname] = contents

pprint.pprint(rulebook)


def traverse_rules(rulebook, bag_contents, target_bag):
    return any((bc[1] == target_bag or traverse_rules(rulebook, rulebook[bc[1]], target_bag) for bc in bag_contents))


def traverse_sum(rulebook, target_bag):
    target_contents = rulebook[target_bag]
    if len(target_contents) == 0:
        print('traverse_sum', target_bag, target_contents, 1)
        return 1
...
ngold = 0
for k in rulebook:
    isvalid = traverse_rules(rulebook, rulebook[k], 'shiny gold')
    print(k, isvalid)
    if isvalid:
        ngold += 1
