...
# step 1
# answered_sets = (set(re.sub(r'\W', '', g.strip())) for g in groups)
# answered_counts = (len(set) for set in answered_sets)

answer_group_sizes = ((len(g.split('\n')), re.sub(r'\W', '', g.strip())) for g in groups)
answer_counts = ([g[1].count(c) == g[0] for c in set(g[1])].count(True) for g in answer_group_sizes)

print(sum(answer_counts))
# print(sum(answered_counts))
