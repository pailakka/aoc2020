...
eyecolors = set('amb blu brn gry grn hzl oth'.split(' '))
validators = {
...
for passport in passports:
    passport = re.split('\s', passport.strip())
    passfields = dict((p.split(':') for p in passport))

    passkeys = set(passfields.keys())
    missingkeys = fields - passkeys
    realmissing = missingkeys - optional
    isvalid = len(realmissing) == 0
    print('keys', passkeys, isvalid)

    if not isvalid:
        continue

    for k, v in passfields.items():
        if k in validators:
